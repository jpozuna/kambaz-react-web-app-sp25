export default [
  {
    _id: "M101",
    name: "Introduction to Rocket Propulsion",
    description: "Basic principles of rocket propulsion and rocket engines.",
    course: "RS101",
    lessons: [
      {
        _id: "L101",
        name: "History of Rocketry",
        description: "A brief history of rocketry and space exploration.",
        module: "M101"
      },
      {
        _id: "L102",
        name: "Rocket Propulsion Fundamentals",
        description: "Basic principles of rocket propulsion.",
        module: "M101"
      },
      {
        _id: "L103",
        name: "Rocket Engine Types",
        description: "Overview of different types of rocket engines.",
        module: "M101"
      }
    ]
  },
  {
    _id: "M102",
    name: "Fuel and Combustion",
    description: "Understanding rocket fuel, combustion processes, and efficiency.",
    course: "RS101",
    lessons: [
      {
        _id: "L201",
        name: "Rocket Fuel",
        description: "Overview of different types of rocket fuels.",
        module: "M102"
      },
      {
        _id: "L202",
        name: "Combustion Processes",
        description: "Understanding combustion processes and efficiency.",
        module: "M102"
      },
      {
        _id: "L203",
        name: "Combustion Instability",
        description: "Understanding combustion instability and mitigation.",
        module: "M102"
      }
    ]
  },
  {
    _id: "M103",
    name: "Nozzle Design",
    description: "Principles of rocket nozzle design and performance optimization.",
    course: "RS101",
    lessons: [
      {
        _id: "L301",
        name: "Nozzle Design",
        description: "Overview of different types of rocket nozzles.",
        module: "M103"
      },
      {
        _id: "L302",
        name: "Nozzle Performance",
        description: "Understanding nozzle performance and efficiency.",
        module: "M103"
      },
      {
        _id: "L303",
        name: "Nozzle Optimization",
        description: "Optimizing nozzle design for specific applications.",
        module: "M103"
      }
    ]
  },
  {
    _id: "M201",
    name: "Fundamentals of Aerodynamics",
    description: "Basic aerodynamic concepts and fluid dynamics principles.",
    course: "RS102"
  },
  {
    _id: "M202",
    name: "Subsonic and Supersonic Flow",
    description: "Understanding subsonic and supersonic aerodynamic behaviors.",
    course: "RS102"
  },
  {
    _id: "M203",
    name: "Aerodynamic Heating",
    description: "Study of aerodynamic heating and thermal protection systems.",
    course: "RS102"
  },
  {
    _id: "M301",
    name: "Spacecraft Structural Design",
    description: "Fundamentals of designing spacecraft structures and materials selection.",
    course: "RS103"
  },
  {
    _id: "M302",
    name: "Orbital Mechanics",
    description: "Understanding orbital dynamics and mission planning.",
    course: "RS103"
  },
  {
    _id: "M303",
    name: "Spacecraft Systems Engineering",
    description: "Overview of spacecraft systems and subsystems engineering.",
    course: "RS103"
  },
  {
    _id: "M201",
    name: "Fundamentals of Aerodynamics",
    description: "Basic aerodynamic concepts and fluid dynamics principles.",
    course: "RS102",
    week: 1,
    lessons: [
      {
        _id: "L401",
        name: "Introduction to Fluid Dynamics",
        description: "Fundamentals of fluid flow and its applications in aerodynamics.",
        module: "M201"
      }
    ]
  },
  {
    _id: "M202",
    name: "Subsonic and Supersonic Flow",
    description: "Understanding subsonic and supersonic aerodynamic behaviors.",
    course: "RS102",
    week: 2,
    lessons: [
      {
        _id: "L402",
        name: "Shock Waves and Expansion Fans",
        description: "Understanding the physics behind shock waves and supersonic flow.",
        module: "M202"
      }
    ]
  },
  {
    _id: "M203",
    name: "Aerodynamic Heating",
    description: "Study of aerodynamic heating and thermal protection systems.",
    course: "RS102",
    week: 3,
    lessons: [
      {
        _id: "L403",
        name: "Thermal Protection Systems",
        description: "Exploring heat-resistant materials and cooling techniques.",
        module: "M203"
      }
    ]
  },
  {
    _id: "M301",
    name: "Spacecraft Structural Design",
    description: "Fundamentals of designing spacecraft structures and materials selection.",
    course: "RS103",
    week: 1,
    lessons: [
      {
        _id: "L501",
        name: "Material Selection for Spacecraft",
        description: "Choosing the right materials for spacecraft design.",
        module: "M301"
      }
    ]
  },
  {
    _id: "M302",
    name: "Orbital Mechanics",
    description: "Understanding orbital dynamics and mission planning.",
    course: "RS103",
    week: 2,
    lessons: [
      {
        _id: "L502",
        name: "Kepler's Laws and Orbital Elements",
        description: "Fundamentals of satellite motion and orbital mechanics.",
        module: "M302"
      }
    ]
  },
  {
    _id: "M303",
    name: "Spacecraft Systems Engineering",
    description: "Overview of spacecraft systems and subsystems engineering.",
    course: "RS103",
    week: 3,
    lessons: [
      {
        _id: "L503",
        name: "Subsystems of a Spacecraft",
        description: "Understanding power, thermal, communication, and control systems.",
        module: "M303"
      }
    ]
  },
  {
    _id: "M401",
    name: "Satellite Communication Systems",
    description: "Basics of satellite communications, frequencies, and link budgets.",
    course: "RS104",
    week: 1,
    lessons: [
      {
        _id: "L601",
        name: "Introduction to Satellite Communication",
        description: "Fundamentals of how satellites are used for communication.",
        module: "M401"
      }
    ]
  },
  {
    _id: "M501",
    name: "Human Spaceflight and Life Support Systems",
    description: "Challenges of human spaceflight and life support in space.",
    course: "RS105",
    week: 2,
    lessons: [
      {
        _id: "L701",
        name: "Life Support Systems",
        description: "Understanding oxygen, water, and waste management in space.",
        module: "M501"
      }
    ]
  }
]; 