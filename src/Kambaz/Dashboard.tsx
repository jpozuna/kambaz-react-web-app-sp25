...
export default function Dashboard({
    ...
    const isEnrolled = (courseId) => {
        if (!Array.isArray(enrollments)) {
            console.error("Enrollments is undefined or not an array:", enrollments);
            return false;
        }
    ...
    return (
        <div id="wd-dashboard" className="container-fluid">
            ...
            <div className="row row-cols-1 row-cols-sm-2 row-cols-md-3 row-cols-lg-4 g-4">
                {courses.filter(course => enrollmentsViewAll || isEnrolled(course._id)).map((course) => (
                    <div key={course._id} className="col" style={{ width: "260px" }}>
                        <div className="card rounded-3 overflow-hidden">
                            <Link to={`/Kambaz/Courses/${course._id}/Home`} className="text-decoration-none text-dark">
                                <img src="/images/canvas-image.png" width="100%" height={160} alt="Course" />
                            ...
                            <div className="card-body">
                                {isEnrolled(course._id) ? (
                                    <Button className="btn btn-danger w-100" onClick={() => handleUnenroll(course._id)}>
                                        Unenroll
                                    </Button>
                                ) : (
                                    <Button className="btn btn-success w-100" onClick={() => handleEnroll(course._id)}>
                                        Enroll
                                    </Button>
                                )}
                            </div>
